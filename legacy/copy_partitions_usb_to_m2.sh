...
show_help() {
    ...
    echo
    echo "Copy contents of partitions from USB SSD to M.2 SSD."
    echo
    echo "Options:"
    echo "  -s, --source      Source disk (default: /dev/sda)"
...
# Helper to build partition device names
build_part_name() {
    local DEVICE="$1"
    local NUM="$2"
...
for SOURCE_PART in $SOURCE_PARTS; do
    ...
    # Match the corresponding destination partition (FIXED: use destination type, not source type)
    DEST_PART=$(build_part_name "$DESTINATION" "$PART_NUM")
    ...
    if [[ "$SRC_FSTYPE" == "ext4" && "$SRC_MOUNT" == "/" ]]; then
        ...
        echo "Detected boot/EFI partition. Copying all boot files."
